...
class TestEpisodes:
    async def test_save_and_get(self, store):
        ep = Episode(id="ep1", group_id="user-1", content="Hello", source="telegram")
    ...
    async def test_recent_episodes(self, store):
        for i in range(5):
            await store.save_episode(Episode(
                id=f"ep{i}", group_id="user-1",
                reference_time=f"2026-03-{20+i}T10:00:00",
            ))

        recent = await store.get_recent_episodes("user-1", limit=3)
        assert len(recent) == 3
        # Most recent first
    ...
    async def test_recent_episodes_with_before(self, store):
        ...
        recent = await store.get_recent_episodes("user-1", limit=3, before="2026-03-23T10:00:00")
        assert len(recent) == 3
...
class TestEntities:
    async def test_save_and_get(self, store):
        ent = Entity(id="ent1", group_id="user-1", name="Alice", entity_type=EntityType.PERSON)
    ...
    async def test_search_by_name(self, store):
        ...
        results = await store.search_entities_by_name("user-1", "alice")
    ...
    async def test_search_by_embedding(self, store):
        ...
        results = await store.search_entities_by_embedding("user-1", [0.9, 0.1, 0.0])
        assert len(results) == 2
        assert results[0].name == "Alice"  # Closer to query

    async def test_save_entities_bulk(self, store):
        entities = [
            Entity(id=f"e{i}", group_id="user-1", name=f"Entity {i}")
            for i in range(10)
        ]
        await store.save_entities(entities)

        loaded = await store.get_entities_by_group("user-1")
...
class TestRelations:
    async def test_save_and_get(self, store):
        ...
        assert loaded is not None
        assert loaded.fact == "Alice lives in London"
    ...
    async def test_get_relations_between(self, store):
        ...
        between = await store.get_relations_between("e1", "e2", group_id="user-1")
    ...
    async def test_get_relations_between_bidirectional(self, store):
        ...
        between = await store.get_relations_between("e2", "e1", group_id="user-1")
...
class TestTemporalFiltering:
    async def test_active_relations_excludes_invalidated(self, store):
        await store.save_relation(Relation(
            id="r1", group_id="user-1", fact="Active fact",
            valid_at="2026-01-01T00:00:00",
        ))
        await store.save_relation(Relation(
            id="r2", group_id="user-1", fact="Invalidated fact",
            valid_at="2025-01-01T00:00:00",
            invalid_at="2026-01-01T00:00:00",
        ))

        active = await store.get_active_relations("user-1")
        assert len(active) == 1
        assert active[0].id == "r1"

    async def test_active_relations_excludes_expired(self, store):
        await store.save_relation(Relation(
            id="r1", group_id="user-1", fact="Active",
        ))
        await store.save_relation(Relation(
            id="r2", group_id="user-1", fact="Expired",
    ...
    async def test_invalidate_relation(self, store):
        await store.save_relation(Relation(
            id="r1", group_id="user-1", fact="Will be invalidated",
            valid_at="2025-01-01T00:00:00",
        ))
    ...
    async def test_invalidate_with_expiry(self, store):
        await store.save_relation(Relation(
            id="r1", group_id="user-1", fact="Will be expired",
        ))
    ...
    async def test_search_with_temporal_window(self, store):
        await store.save_relation(Relation(
            id="r1", group_id="user-1", fact="Old fact about London",
            valid_at="2020-01-01T00:00:00",
        ))
        await store.save_relation(Relation(
            id="r2", group_id="user-1", fact="New fact about London",
            valid_at="2026-01-01T00:00:00",
        ))

        filters = SearchFilters(valid_at_start="2025-01-01T00:00:00")
        results = await store.search_relations_by_text("user-1", "london", filters=filters)
        assert len(results) == 1
    ...
    async def test_search_includes_invalidated_when_requested(self, store):
        ...
        results = await store.search_relations_by_text("user-1", "fact")
        assert len(results) == 1

        # With flag: include invalidated
        filters = SearchFilters(include_invalidated=True)
        results = await store.search_relations_by_text("user-1", "fact", filters=filters)
        assert len(results) == 2
...
class TestPartitionIsolation:
    ...
    async def test_search_isolated_by_group(self, store):
        ...
        assert len(results) == 1
        assert results[0].relation.group_id == "user-1"
...
class TestEpisodicLinks:
    async def test_save_links(self, store):
        links = [
            EpisodicLink(episode_id="ep1", entity_id="e1", group_id="user-1"),
            EpisodicLink(episode_id="ep1", entity_id="e2", group_id="user-1"),
        ]
...
class TestVectorSearch:
    async def test_search_by_embedding(self, store):
        await store.save_relation(Relation(
            id="r1", group_id="user-1", fact="Alice lives in London",
    ...
    async def test_vector_search_respects_temporal_filters(self, store):
        ...
        await store.save_relation(Relation(
            id="r2", group_id="user-1", fact="Invalidated",
            fact_embedding=[0.9, 0.1],
            invalid_at="2026-01-01T00:00:00",
        ))

        results = await store.search_relations_by_embedding("user-1", [1.0, 0.0])
        assert len(results) == 1
...
class TestRoundTrip:
    async def test_full_lifecycle(self, store):
        ...
        # Create entities
        dom = Entity(id="e1", group_id="user-1", name="Alice", entity_type=EntityType.PERSON)
        london = Entity(id="e2", group_id="user-1", name="London", entity_type=EntityType.LOCATION)
        await store.save_entities([dom, london])
        ...
        london = Entity(id="e3", group_id="user-1", name="London", entity_type=EntityType.LOCATION)
        ...
        active = await store.get_active_relations("user-1")
        assert len(active) == 1
        assert active[0].fact == "Alice lives in London"

        # Old fact still retrievable with flag
        filters = SearchFilters(include_invalidated=True)
        all_rels = await store.search_relations_by_text("user-1", "alice", filters=filters)
